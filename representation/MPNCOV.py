...
class MPNCOV(nn.Module):
    def __init__(self, iterNum=3, is_sqrt=True, is_vec=True, input_dim=640, dimension_reduction=None):

        super(MPNCOV, self).__init__()
        self.iterNum = iterNum
        self.is_sqrt = is_sqrt
        self.is_vec = is_vec
        if dimension_reduction == input_dim:
            self.dr = None
        else:
...
class Sqrtm(Function):
    ...
    def forward(ctx, input, iterN):
        ...
        Y = torch.zeros(batchSize, iterN - 1, dim, dim, requires_grad=False, device=x.device).type(dtype)  # .half()
        Z = torch.eye(dim, dim, requires_grad=False, device=x.device).view(1, dim, dim).repeat(batchSize, iterN - 1, 1,
                                                                                               1).type(dtype)  # .half()
        if iterN < 2:
            ZY = 0.5 * (I3 - A)
            YZY = A.bmm(ZY)
        ...
        ctx.save_for_backward(input, A, YZY, normA, Y, Z, I3)
        ctx.iterN = iterN
        return y

    @staticmethod
    def backward(ctx, grad_output):
        input, A, ZY, normA, Y, Z, I3 = ctx.saved_tensors
        iterN = ctx.iterN
        x = input
        batchSize = x.data.shape[0]
        dim = x.data.shape[1]
        dtype = x.dtype
        der_postCom = grad_output * torch.sqrt(normA).view(batchSize, 1, 1).expand_as(x).half()
        ...
        for i in range(batchSize):
            grad_input[i, :, :] += (der_postComAux[i] - grad_aux[i] / (normA[i] * normA[i])) * torch.eye(dim,
